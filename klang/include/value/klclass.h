...
typedef KlObject* (*KlObjectConstructor)(KlClass* klclass, KlMM* klmm);
...
static inline KlObject* klclass_new_object(KlClass* klclass, KlMM* klmm);
...
static inline KlObject* klclass_new_object(KlClass* klclass, KlMM* klmm) {
  return klclass->constructor(klclass, klmm);
}

static inline KlException klclass_newlocal(KlClass* klclass, KlMM* klmm, KlString* key) {
  KlValue tmpval;
  klvalue_setid(&tmpval, klclass->nlocal++);
  KlException exception = klclass_set(klclass, klmm, key, &tmpval);
  if (kl_unlikely(exception)) --klclass->nlocal;
  return exception;
}

static inline KlException klclass_newshared(KlClass* klclass, KlMM* klmm, KlString* key, KlValue* value) {
  return klclass_set(klclass, klmm, key, value);
}
...
static inline KlValue* klobject_getfield(KlObject* object, KlString* key) {
  ...
  if (klclass_is_local(ceil)) {
    return object->attrs + klvalue_getid(&ceil->value);
  } else {
    return &ceil->value;
  }
}

  ...
  while (true) {
    ...
    switch (opcode) {
      ...
      case KLOPCODE_METHOD: {
        ...
        /* obj.foo(args) should behave as: foo([obj,] args) where let foo = obj.foo;
         * so method that does not exist should be treated as nil. */
        if (kl_unlikely(!callable)) klvalue_setnil(callable);

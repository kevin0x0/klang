...
static KlCodeVal klgen_pushrelinst(KlGenUnit* gen, KlCstBin* relcst, size_t leftid, size_t rightid, bool jumpcond) {
  switch (relcst->op) {
    case KLTK_LT: {
      klgen_emit(gen, klinst_lt(leftid, rightid), klgen_cstposition(relcst));
      break;
    }
    case KLTK_LE: {
      klgen_emit(gen, klinst_le(leftid, rightid), klgen_cstposition(relcst));
      break;
    }
    case KLTK_GT: {
      klgen_emit(gen, klinst_gt(leftid, rightid), klgen_cstposition(relcst));
      break;
    }
    case KLTK_GE: {
      klgen_emit(gen, klinst_ge(leftid, rightid), klgen_cstposition(relcst));
      break;
    }
    case KLTK_EQ: {
      klgen_emit(gen, klinst_eq(leftid, rightid), klgen_cstposition(relcst));
      break;
    }
    case KLTK_IS: {
      klgen_emit(gen, klinst_is(leftid, rightid), klgen_cstposition(relcst));
      break;
    }
    case KLTK_NE: {
      klgen_emit(gen, klinst_ne(leftid, rightid), klgen_cstposition(relcst));
      break;
    }
    case KLTK_ISNOT: {
      klgen_emit(gen, klinst_is(leftid, rightid), klgen_cstposition(relcst));
  ...
  size_t pc = klgen_emit(gen, klinst_condjmp(jumpcond, 0), klgen_cstposition(relcst));
...
static KlCodeVal klgen_pushrelinsti(KlGenUnit* gen, KlCstBin* relcst, size_t leftid, KlCInt imm, bool jumpcond) {
  switch (relcst->op) {
    ...
    case KLTK_LE: {
      klgen_emit(gen, klinst_lei(leftid, imm), klgen_cstposition(relcst));
      break;
    }
    case KLTK_GT: {
      klgen_emit(gen, klinst_gti(leftid, imm), klgen_cstposition(relcst));
      break;
    }
    case KLTK_GE: {
      klgen_emit(gen, klinst_gei(leftid, imm), klgen_cstposition(relcst));
      break;
    }
    case KLTK_EQ: {
      if (jumpcond) {
        klgen_emit(gen, klinst_eqi(leftid, imm), klgen_cstposition(relcst));
      } else {
        klgen_emit(gen, klinst_nei(leftid, imm), klgen_cstposition(relcst));
      }
      size_t pc = klgen_emit(gen, klinst_condjmp(true, 0), klgen_cstposition(relcst));
      return klcodeval_jmplist(pc);
    }
    case KLTK_NE: {
      if (jumpcond) {
...
static KlCodeVal klgen_pushrelinstc(KlGenUnit* gen, KlCstBin* relcst, size_t leftid, size_t conidx, bool jumpcond) {
  switch (relcst->op) {
    case KLTK_LT: {
      klgen_emit(gen, klinst_ltc(leftid, conidx), klgen_cstposition(relcst));
      break;
    }
    case KLTK_LE: {
      klgen_emit(gen, klinst_lec(leftid, conidx), klgen_cstposition(relcst));
      break;
    }
    case KLTK_GT: {
      klgen_emit(gen, klinst_gtc(leftid, conidx), klgen_cstposition(relcst));
      break;
    }
    case KLTK_GE: {
      klgen_emit(gen, klinst_gec(leftid, conidx), klgen_cstposition(relcst));
      break;
    }
    case KLTK_EQ: {
      if (jumpcond) {
        klgen_emit(gen, klinst_eqc(leftid, conidx), klgen_cstposition(relcst));
      } else {
        klgen_emit(gen, klinst_nec(leftid, conidx), klgen_cstposition(relcst));
      }
      size_t pc = klgen_emit(gen, klinst_condjmp(true, 0), klgen_cstposition(relcst));
      return klcodeval_jmplist(pc);
    }
...
static KlCodeVal klgen_exprrelleftliteral(KlGenUnit* gen, KlCstBin* bincst, KlCodeVal left, bool jumpcond) {
  ...
  if (right.kind == KLVAL_STACK) {
    ...
    return klgen_exprrelrightnonstk(gen, bincst, oristktop, left, right, jumpcond);
  }
}
  ...
  switch (right.kind) {
    case KLVAL_INTEGER: {
      KlCodeVal res = klinst_inrange(right.intval, 16)
                    ? klgen_pushrelinsti(gen, bincst, left.index, right.intval, jumpcond)
                    : klgen_pushrelinstc(gen, bincst, left.index, klgen_newinteger(gen, right.intval), jumpcond);
      klgen_stackfree(gen, oristktop);
      return res;
    }
    case KLVAL_FLOAT: {
      KlCodeVal res = klgen_pushrelinstc(gen, bincst, left.index, klgen_newfloat(gen, right.floatval), jumpcond);
      klgen_stackfree(gen, oristktop);
      return res;
    }
    case KLVAL_STRING: {
      KlCodeVal res = klgen_pushrelinstc(gen, bincst, left.index, klgen_newstring(gen, right.string), jumpcond);
      klgen_stackfree(gen, oristktop);
      return res;
    }
    case KLVAL_REF:
...
KlCodeVal klgen_exprrelation(KlGenUnit* gen, KlCstBin* relcst, bool jumpcond) {
  ...
  if (klcodeval_isconstant(left)) {
    ...
    klgen_putonstack(gen, &left, klgen_cstposition(relcst->loperand));
  }
...
KlCodeVal klgen_exprbool(KlGenUnit* gen, KlCst* cst, bool jumpcond) {
  ...
  size_t pc = klgen_emit(gen, jumpcond ? klinst_truejmp(res.index, 0) : klinst_falsejmp(res.index, 0), klgen_cstposition(cst));
...
KlCodeVal klgen_expror(KlGenUnit* gen, KlCstBin* orcst, bool jumpcond) {
  ...
  if (klcodeval_isconstant(rjmp)) {
    size_t stktop = klgen_stacktop(gen);
    klgen_putonstack(gen, &rjmp, klgen_cstposition(orcst->roperand));
...
KlCodeVal klgen_exprand(KlGenUnit* gen, KlCstBin* andcst, bool jumpcond) {
  ...
  if (klcodeval_isconstant(rjmp)) {
    size_t stktop = klgen_stacktop(gen);
    klgen_putonstack(gen, &rjmp, klgen_cstposition(andcst->roperand));
    rjmp = klcodeval_jmplist(klgen_emit(gen,
                                        jumpcond ? klinst_truejmp(rjmp.index, 0)
                                                 : klinst_falsejmp(rjmp.index, 0),
...
static KlCodeVal klgen_exprorset(KlGenUnit* gen, KlCstBin* orcst, size_t target, bool setcond) {
  if (setcond) {
    ...
    if (klcodeval_istrue(rval)) { /* true, set value and jump over */
      size_t stktop = klgen_stacktop(gen);
      klgen_loadval(gen, target, rval, klgen_cstposition(orcst->roperand));
      size_t pc = klgen_emit(gen, klinst_jmp(0), klgen_cstposition(orcst->roperand));
      klgen_mergejmplist_maynone(gen, &gen->info.jumpinfo->terminatelist, klcodeval_jmplist(pc));
      klgen_stackfree(gen, stktop);
    }
...
static KlCodeVal klgen_exprandset(KlGenUnit* gen, KlCstBin* andcst, size_t target, bool setcond) {
  if (setcond) {
    ...
    if (klcodeval_istrue(rval)) { /* true, set value and jump over */
      size_t stktop = klgen_stacktop(gen);
      klgen_loadval(gen, target, rval, klgen_cstposition(andcst->roperand));
      size_t pc = klgen_emit(gen, klinst_jmp(0), klgen_cstposition(andcst->roperand));
      klgen_mergejmplist_maynone(gen, &gen->info.jumpinfo->terminatelist, klcodeval_jmplist(pc));
      klgen_stackfree(gen, stktop);
    }
    /* else false, fall through */
...
static KlCodeVal klgen_exprboolset(KlGenUnit* gen, KlCst* cst, size_t target, bool setcond) {
  ...
  klgen_mergejmplist_maynone(gen, &gen->info.jumpinfo->terminatelist, klcodeval_jmplist(terminatepc));
  klgen_stackfree(gen, stktop);
  return klcodeval_none();
}
...
static KlCodeVal klgen_exprboolvalraw(KlGenUnit* gen, KlCst* cst, size_t target) {
  if (klcst_kind(cst) == KLCST_EXPR_PRE && klcast(KlCstPre*, cst)->op == KLTK_NOT) {
    ...
    if (klcodeval_isconstant(res)) return res;
    klgen_mergejmplist_maynone(gen, &gen->info.jumpinfo->truelist, res);
    ...
    if (bincst->op == KLTK_AND) {
      ...
      if (klcodeval_isconstant(rjmp)) {
        if (klcodeval_istrue(rjmp)) {
          rjmp = klcodeval_jmplist(klgen_emit(gen, klinst_jmp(0), klgen_cstposition(bincst->roperand)));
          klgen_mergejmplist_maynone(gen, &gen->info.jumpinfo->truelist, rjmp);
        }
        klgen_finishexprboolvalraw(gen, target, klgen_cstposition(cst));
      }
      return klcodeval_none();
  ...
  if (gen->info.jumpinfo->truelist.kind == KLVAL_NONE &&
    ...
    KlCodeVal jmp = klcodeval_jmplist(klgen_emit(gen, klinst_jmp(0), klgen_cstposition(cst)));
    klgen_mergejmplist_maynone(gen, &gen->info.jumpinfo->terminatelist, jmp);
    klgen_finishexprboolvalraw(gen, target, klgen_cstposition(cst));
  }

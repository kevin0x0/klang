...
void klgen_exprarrgen(KlGenUnit* gen, KlCstArrayGenerator* arrgencst, size_t target) {
  ...
  klgen_emit(gen, klinst_mkarray(stktop, 0, 0), klgen_cstposition(arrgencst));
...
void klgen_exprvararg(KlGenUnit* gen, KlCstVararg* varargcst, size_t nwanted, size_t target) {
  if (!gen->vararg) {
    klgen_error(gen, klcst_begin(varargcst), klcst_end(varargcst),
...
static bool klgen_exprhasverres(KlGenUnit* gen, KlCst* expr) {
  switch (klcst_kind(expr)) {
    ...
    case KLCST_EXPR_WHERE: {
      return klgen_exprhasverres(gen, klcast(KlCstWhere*, expr)->expr);
    }
...
static size_t klgen_exprwhere(KlGenUnit* gen, KlCstWhere* wherecst, size_t nwanted, size_t target) {
  ...
  if (nwanted == KLINST_VARRES && !klgen_exprhasverres(gen, klcast(KlCstWhere*, wherecst)->expr))
    nwanted = 1;
  ...
  if (nwanted == KLINST_VARRES) {
    ...
    size_t stktop = klgen_stacktop(gen);
    size_t nres = klgen_takeall(gen, wherecst->expr, stktop);
...
void klgen_exprclass(KlGenUnit* gen, KlCstClass* classcst, size_t target) {
  ...
  if (hasbase) /* base is specified */
    klgen_exprtarget_noconst(gen, classcst->baseclass, stktop);
...
static KlCodeVal klgen_identifier(KlGenUnit* gen, KlCstIdentifier* idcst) {
  ...
  if (symbol) {
    ...
    size_t stkid = klgen_stackalloc1(gen);
    size_t conidx = klgen_newstring(gen, idcst->id);
    klgen_emit(gen, klinst_loadglobal(stkid, conidx), klgen_cstposition(idcst));
...
void klgen_exprcall(KlGenUnit* gen, KlCstCall* callcst, size_t nret, size_t target) {
  if (klcst_kind(callcst->callable) == KLCST_EXPR_DOT) {
    KlCstDot* dotcst = klcast(KlCstDot*, callcst->callable);
    klgen_method(gen, dotcst->operand, dotcst->field, callcst->args, klgen_cstposition(callcst), nret, target);
    return;
  }
  size_t base = klgen_stacktop(gen);
...
void klgen_multival(KlGenUnit* gen, KlCst* cst, size_t nval, size_t target) {
  ...
  switch (klcst_kind(cst)) {
    ...
    case KLCST_EXPR_CALL: {
      klgen_exprcall(gen, klcast(KlCstCall*, cst), nval, target);
      break;
    }
    case KLCST_EXPR_WHERE: {
    ...
    case KLCST_EXPR_TUPLE: {
      size_t stktop = klgen_stacktop(gen);
      KlCst** elems = klcast(KlCstTuple*, cst)->elems;
      size_t nelem = klcast(KlCstTuple*, cst)->nelem;
      if (nelem == 0) {
        klgen_emitloadnils(gen, target, nval, klgen_cstposition(cst));
        klgen_stackfree(gen, target + nval > stktop ? target + nval : stktop);
        break;
      }
...
size_t klgen_trytakeall(KlGenUnit* gen, KlCst* cst, KlCodeVal* val) {
  switch (klcst_kind(cst)) {
    ...
    case KLCST_EXPR_CALL: {
      size_t stktop = klgen_stacktop(gen);
      klgen_exprcall(gen, klcast(KlCstCall*, cst), KLINST_VARRES, stktop);
      *val = klcodeval_stack(stktop);
      return KLINST_VARRES;
    }
    case KLCST_EXPR_WHERE: {
...
size_t klgen_takeall(KlGenUnit* gen, KlCst* cst, size_t target) {
  switch (klcst_kind(cst)) {
    ...
    case KLCST_EXPR_CALL: {
      klgen_exprcall(gen, klcast(KlCstCall*, cst), KLINST_VARRES, target);
      return KLINST_VARRES;
    }
    case KLCST_EXPR_WHERE: {
...
static KlInstruction klgen_bininsti(KlCstBin* bincst, size_t stkid, size_t leftid, KlCInt imm) {
  switch (bincst->op) {
    case KLTK_ADD:
    ...
    switch (bincst->op) {
      ...
      case KLTK_MOD: {
        ...
        if (klinst_inurange(conidx, 8)) {
          klgen_emit(gen, klinst_modc(target, left.index, conidx), klgen_cstposition(bincst));
        } else {
          size_t tmp = klgen_stackalloc1(gen);
          klgen_emit(gen, klinst_loadc(tmp, conidx), klgen_cstposition(bincst->roperand));
      ...
      case KLTK_IDIV: {
        ...
        size_t conidx = klgen_newfloat(gen, r);
        if (klinst_inurange(conidx, 8)) {
          klgen_emit(gen, klinst_idivc(target, left.index, conidx), klgen_cstposition(bincst));
        } else {
          size_t tmp = klgen_stackalloc1(gen);
          klgen_emit(gen, klinst_loadc(tmp, conidx), klgen_cstposition(bincst->roperand));
...
static KlCodeVal klgen_exprbinrightnonstk(KlGenUnit* gen, KlCstBin* bincst, KlCodeVal left, KlCodeVal right, size_t target, size_t oristktop) {
  ...
  switch (right.kind) {
    ...
    case KLVAL_STRING: {
      ...
      if (klinst_inurange(conidx, 8) && bincst->op != KLTK_CONCAT) {
        klgen_emit(gen, klgen_bininstc(bincst, target, left.index, conidx), klgen_cstposition(bincst));
      } else {
        size_t stktop = klgen_stackalloc1(gen);
        klgen_emit(gen, klinst_loadc(stktop, conidx), klgen_cstposition(bincst->roperand));
...
static KlCodeVal klgen_exprappend(KlGenUnit* gen, KlCstPost* postcst) {
  ...
  KlCstTuple* tuple = klcast(KlCstTuple*, postcst->post);
  if (tuple->nelem == 1 && klcst_kind(tuple->elems[0]) == KLCST_EXPR_ID) {
...
KlCodeVal klgen_exprpost(KlGenUnit* gen, KlCstPost* postcst, size_t target, bool append_target) {
  switch (postcst->op) {
    ...
    case KLTK_APPEND: {
      size_t stktop = klgen_stacktop(gen);
      KlCodeVal appendable = klgen_exprappend(gen, postcst);
...
static void klgen_exprnew(KlGenUnit* gen, KlCstNew* newcst, size_t target) {
  ...
  klgen_emitmethod(gen, stktop, conidx, narg, 0, stktop, klgen_cstposition(newcst));
  klgen_stackfree(gen, oristktop == target ? target + 1 : oristktop);
}
...
static void klgen_exprdot(KlGenUnit* gen, KlCstDot* dotcst, size_t target) {
  ...
  if (obj.kind == KLVAL_REF) {
    if (klinst_inurange(conidx, 8)) {
      klgen_emit(gen, klinst_refgetfieldc(target, obj.index, conidx), klgen_cstposition(dotcst));
    } else {
      size_t stkid = klgen_stackalloc1(gen);
      klgen_emit(gen, klinst_loadc(stkid, conidx), klgen_cstposition(dotcst));
    ...
    if (klinst_inurange(conidx, 8)) {
      klgen_emit(gen, klinst_getfieldc(target, obj.index, conidx), klgen_cstposition(dotcst));
    } else {
      size_t stkid = klgen_stackalloc1(gen);
      klgen_emit(gen, klinst_loadc(stkid, conidx), klgen_cstposition(dotcst));
...
KlCodeVal klgen_expr(KlGenUnit* gen, KlCst* cst) {
  switch (klcst_kind(cst)) {
    ...
    case KLCST_EXPR_CALL: {
      size_t target = klgen_stacktop(gen);   /* here we generate the value on top of the stack */
      klgen_exprcall(gen, klcast(KlCstCall*, cst), 1, target);
      return klcodeval_stack(target);
    }
    case KLCST_EXPR_DOT: {
      size_t target = klgen_stacktop(gen);   /* here we generate the value on top of the stack */
  ...
  switch (klcst_kind(cst)) {
    ...
    case KLCST_EXPR_ID: {
      ...
      if (symbol) {
        ...
        size_t conidx = klgen_newstring(gen, idcst->id);
        klgen_emit(gen, klinst_loadglobal(target, conidx), klgen_cstposition(idcst));
      }
      klgen_stackfree(gen, stktop == target ? target + 1 : stktop);
      return klcodeval_stack(target);
    }

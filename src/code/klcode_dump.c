...
kl_noreturn static void dump_failure(KlCodeDumpState* state);
static void dump_code(KlCodeDumpState* state, const KlCode* code);
static void dumpobj(KlCodeDumpState* state, const void* obj, size_t size);
static void dump_globalheader(KlCodeDumpState* state);
static void dump_codeheader(KlCodeDumpState* state, const KlCodeHeader* header);
static void dump_refinfo(KlCodeDumpState* state, const KlCRefInfo* refinfo, size_t nref);
static void dumpinteger(KlCodeDumpState* state, KlCInt val);
static void dumpfloat(KlCodeDumpState* state, KlCFloat val);
static void dumpbool(KlCodeDumpState* state, KlCBool val);
static void dumpnil(KlCodeDumpState* state);
static void dumpstring(KlCodeDumpState* state, const KlStrTbl* strtbl, KlStrDesc str);
static void dump_constants(KlCodeDumpState* state, const KlStrTbl* strtbl, const KlConstant* constants, size_t nconst);
static void dump_instructions(KlCodeDumpState* state, const KlInstruction* inst, size_t codelen);
static void dump_posinfo(KlCodeDumpState* state, KlFilePosition* posinfo, size_t npos);
...
bool klcode_dump(const KlCode* code, Ko* file) {
  ...
  if (setjmp(dumpstate.jmppos) == 0) {
    dump_globalheader(&dumpstate);
    dumpstring(&dumpstate, code->strtbl, code->srcfile);
    dump_code(&dumpstate, code);
    return true;
  } else {
    return false;
  }
...
static void dump_code(KlCodeDumpState* state, const KlCode* code) {
  ...
  memset(header.reserved, 0, sizeof (header.reserved));
  dump_codeheader(state, &header);
  dump_refinfo(state, code->refinfo, header.nref);
  dump_constants(state, code->strtbl, code->constants, header.nconst);
  dump_instructions(state, code->code, header.codelen);
  if (header.hasposinfo)
    dump_posinfo(state, code->posinfo, header.codelen);
  for (size_t i = 0; i < code->nnested; ++i)
    dump_code(state, code->nestedfunc[i]);
}
...
static void dump_constants(KlCodeDumpState* state, const KlStrTbl* strtbl, const KlConstant* constants, size_t nconst) {
  for (size_t i = 0; i < nconst; ++i) {
    switch (constants[i].type) {
      case KLC_INT: {
        dumpinteger(state, constants[i].intval);
        break;
      }
      case KLC_FLOAT: {
        dumpfloat(state, constants[i].floatval);
        break;
      }
      case KLC_BOOL: {
        dumpbool(state, constants[i].boolval);
        break;
      }
      case KLC_NIL: {
        dumpnil(state);
      ...
      case KLC_STRING: {
        dumpstring(state, strtbl, constants[i].string);
        break;
      }
      default: {
...
static KlUnDumpError undump_globalheader(KlCodeUndumpState* state);
static KlCode* undump_code(KlCodeUndumpState* state);
static KlUnDumpError undumpobj(KlCodeUndumpState* state, void* buf, size_t size);
static KlUnDumpError undumpinteger(KlCodeUndumpState* state, KlCInt* buf);
static KlUnDumpError undumpfloat(KlCodeUndumpState* state, KlCFloat* buf);
static KlUnDumpError undumpbool(KlCodeUndumpState* state, KlCBool* buf);
static KlUnDumpError undumpstring_withtypetag(KlCodeUndumpState* state, KlStrDesc* strdesc);
static KlUnDumpError undumpstring(KlCodeUndumpState* state, KlStrDesc* strdesc);
static KlUnDumpError undump_refinfo(KlCodeUndumpState* state, KlCRefInfo* refinfo, size_t nref);
static KlUnDumpError undump_constants(KlCodeUndumpState* state, KlConstant* constants, size_t nconst);
static KlUnDumpError undump_instructions(KlCodeUndumpState* state, KlInstruction* insts, size_t codelen);
static KlUnDumpError undump_posinfo(KlCodeUndumpState* state, KlFilePosition* posinfo, size_t codelen);
...
KlCode* klcode_undump(Ki* file, KlStrTbl* strtbl, KlUnDumpError* perror) {
  ...
  KlUnDumpError error = undump_globalheader(&state);
  if (kl_unlikely(error)) {
    if (perror) *perror = error;
    return NULL;
  }
  error = undumpstring_withtypetag(&state, &state.srcfile);
  if (kl_unlikely(error)) {
    if (perror) *perror = error;
    return NULL;
  }
  KlCode* code = undump_code(&state);
  if (kl_unlikely(!code)) {
...
static KlCode* undump_code(KlCodeUndumpState* state) {
  ...
  KlUnDumpError error = undump_refinfo(state, refinfo, header.nref);
  if (kl_likely(error == KLUNDUMP_SUCCESS))
    error = undump_constants(state, constants, header.nconst);
  if (kl_likely(error == KLUNDUMP_SUCCESS))
    error = undump_instructions(state, insts, header.codelen);
  if (kl_likely(error == KLUNDUMP_SUCCESS)) {
    if (header.hasposinfo)
      error = undump_posinfo(state, posinfo, header.codelen);
  }
  if (kl_likely(error == KLUNDUMP_SUCCESS)) {
    error = KLUNDUMP_SUCCESS;
    for (size_t i = 0; i < header.nnested; ++i) {
      KlCode* nested = undump_code(state);
...
static KlUnDumpError undumpinteger(KlCodeUndumpState* state, KlCInt* buf) {
  return undumpobj(state, buf, sizeof (KlCInt));
}

static KlUnDumpError undumpfloat(KlCodeUndumpState* state, KlCFloat* buf) {
  return undumpobj(state, buf, sizeof (KlCFloat));
}

static KlUnDumpError undumpbool(KlCodeUndumpState* state, KlCBool* buf) {
  return undumpobj(state, buf, sizeof (KlCBool));
}
...
static KlUnDumpError undumpstring_withtypetag(KlCodeUndumpState* state, KlStrDesc* strdesc) {
  ...
  if (kl_unlikely(type == KOF || type != KLC_STRING))
    return KLUNDUMP_ERROR_BAD;
  return undumpstring(state, strdesc);
}
...
static KlUnDumpError undump_constants(KlCodeUndumpState* state, KlConstant* constants, size_t nconst) {
  for (size_t i = 0; i < nconst; ++i) {
    ...
    switch (type) {
      case KLC_INT: {
        error = undumpinteger(state, &constants[i].intval);
        break;
      }
      case KLC_FLOAT: {
        error = undumpfloat(state, &constants[i].floatval);
        break;
      }
      case KLC_BOOL: {
        error = undumpbool(state, &constants[i].boolval);
        break;
      }
      case KLC_NIL: {
        break;
      ...
      case KLC_STRING: {
        error = undumpstring(state, &constants[i].string);
        break;
      }
      default: {
        return KLUNDUMP_ERROR_BAD;

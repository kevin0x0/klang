...
static void emit_tuplebinding(KlGenUnit* gen, size_t nval, KlCStkId target, KlCStkId val, KlFilePosition filepos);
static void emit_arraybinding(KlGenUnit* gen, size_t nfront, size_t nback, KlCStkId target, KlCStkId val, KlFilePosition filepos);
static void emit_mapbinding(KlGenUnit* gen, size_t nres, KlCStkId target, KlCStkId val, KlFilePosition filepos);
static void emit_objbinding(KlGenUnit* gen, size_t nres, KlCStkId target, KlCStkId val, KlFilePosition filepos);
static void emit_genericbinding(KlGenUnit* gen, size_t nres, KlCStkId target, KlCIdx method, KlCStkId obj, size_t narg, KlFilePosition filepos);
static void emit_tuplematching(KlGenUnit* gen, size_t nval, KlCStkId target, KlCStkId val, KlFilePosition filepos);
static void emit_arraymatching(KlGenUnit* gen, size_t nfront, size_t nback, KlCStkId target, KlCStkId val, KlFilePosition filepos);
static void emit_mapmatching(KlGenUnit* gen, size_t nres, KlCStkId target, KlCStkId val, KlFilePosition filepos);
static void emit_objmatching(KlGenUnit* gen, size_t nres, KlCStkId target, KlCStkId val, KlFilePosition filepos);
static void emit_genericmatching(KlGenUnit* gen, size_t nres, KlCStkId target, KlCIdx method, KlCStkId obj, size_t narg, KlFilePosition filepos);

static void gen_pattern_constant_matchjmp(KlGenUnit* gen, KlAstConstant* constant);
static void gen_pattern_constant_match(KlGenUnit* gen, KlAstConstant* constant);
...
static KlPatternEmitter binders = {
  .tuple = emit_tuplebinding,
  .array = emit_arraybinding,
  .map = emit_mapbinding,
  .obj = emit_objbinding,
  .generic = emit_genericbinding,
  .constant = gen_pattern_constant_match,
};

static KlPatternEmitter matchers = {
  .tuple = emit_tuplematching,
  .array = emit_arraymatching,
  .map = emit_mapmatching,
  .obj = emit_objmatching,
  .generic = emit_genericmatching,
  .constant = gen_pattern_constant_matchjmp,
};


static void gen_pattern_fast(KlGenUnit* gen, KlAstExpr* pattern, KlPatternEmitter* emitter);
static KlCStkId gen_pattern(KlGenUnit* gen, KlAstExpr* pattern, KlCStkId target, KlPatternEmitter* emitter);
...
static void handle_array(KlGenUnit* gen, KlAstExpr** elems, size_t nelem, size_t* pnfront, size_t* pnback) {
  size_t nfront = 0;
...
static KlStrDesc getmethodname(KlGenUnit* gen, KlAstExpr* ast) {
...
static inline bool islval(KlAstExpr* lval) {
  return klast_expr_islvalue(lval);
}

static inline bool allislval(KlAstExpr** lvals, size_t nlval) {
...
static inline bool allislval_maynull(KlAstExpr** lvals, size_t nlval) {
  for (size_t i = 0; i < nlval; ++i) {
    if (lvals[i] && !islval(lvals[i]))
      return false;
  }
  return true;
}
  ...
  switch (klast_kind(pattern)) {
    case KLAST_EXPR_TUPLE: {
      ...
      for (size_t i = nexpr; i--;)
        target = gen_pattern(gen, exprs[i], target, emitter);
      return target;
    }
    case KLAST_EXPR_ARRAY: {
      ...
      size_t nback;
      KlCStkId obj = klgen_stacktop(gen) - 1;
      handle_array(gen, elems, nelem, &nfront, &nback);
      if (allislval(elems, nfront) &&
          (nback == 0 || allislval(elems + nfront + 1, nelem - nfront - 1))) {
      ...
      if (nback != 0) {
        for (size_t i = nelem; i-- != nfront + 1;)
          target = gen_pattern(gen, elems[i], target, emitter);
      }
    ...
    case KLAST_EXPR_CLASS: {
      ...
      if (allislval_maynull(vals, nval)) {
      ...
      for (size_t i = nval; i--;) {
        ...
        target = gen_pattern(gen, vals[i], target, emitter);
      }
      return target;
    }
    case KLAST_EXPR_MAP: {
    ...
    case KLAST_EXPR_WALRUS: {
      ...
      target = gen_pattern(gen, walrus->rval, target, emitter);
      return gen_pattern(gen, walrus->pattern, target, emitter);
    }
    case KLAST_EXPR_BIN: {
      kl_assert(klgen_stacktop(gen) > 0, "");
      KlStrDesc methodname = getmethodname(gen, pattern);
      KlCIdx method = klgen_newstring(gen, methodname);
      KlCStkId obj = klgen_stacktop(gen) - 1;
      KlAstBin* bin = klcast(KlAstBin*, pattern);
      if (islval(bin->loperand) && islval(bin->roperand)) {
      ...
      target = gen_pattern(gen, bin->roperand, target, emitter);
      return gen_pattern(gen, bin->loperand, target, emitter);
    }
    case KLAST_EXPR_PRE: {
      kl_assert(klgen_stacktop(gen) > 0, "");
      KlStrDesc methodname = getmethodname(gen, pattern);
      ...
      return gen_pattern(gen, pre->operand, target, emitter);
    }
...
static void gen_pattern_fast(KlGenUnit* gen, KlAstExpr* pattern, KlPatternEmitter* emitter) {
  switch (klast_kind(pattern)) {
    case KLAST_EXPR_TUPLE: {
      ...
      klgen_stackfree(gen, obj + nexpr);
      if (nexpr != 0)
        gen_pattern_fast(gen, tuple->vals[tuple->nval - 1], emitter);
      return;
    }
    case KLAST_EXPR_ARRAY: {
      kl_assert(klgen_stacktop(gen) > 0, "");
      ...
      for (size_t i = nelem; i--;) {
        if (klast_kind(elems[i]) != KLAST_EXPR_VARARG) {
          gen_pattern_fast(gen, elems[i], emitter);
          return;
        }
      }
      return;
    ...
    case KLAST_EXPR_CLASS: {
      ...
      klgen_stackfree(gen, obj + nshared);
      if (nshared == 0) return;
      gen_pattern_fast(gen, lastshared, emitter);
      return;
    }
    case KLAST_EXPR_MAP: {
      kl_assert(klgen_stacktop(gen) > 0, "");
      ...
      klgen_stackfree(gen, obj + npair);
      if (npair == 0) return;
      gen_pattern_fast(gen, vals[npair - 1], emitter);
      return;
    }
    case KLAST_EXPR_WALRUS: {
      kl_assert(klgen_stacktop(gen) > 0, "");
      ...
      KlCStkId copyobj = klgen_stackalloc1(gen);
      klgen_emitmove(gen, copyobj, copyobj - 1, 1, klgen_astposition(walrus));
      gen_pattern_fast(gen, walrus->rval, emitter);
      return;
    }
    case KLAST_EXPR_BIN: {
      kl_assert(klgen_stacktop(gen) > 0, "");
      ...
      klgen_stackfree(gen, obj + 2);
      KlAstBin* bin = klcast(KlAstBin*, pattern);
      gen_pattern_fast(gen, bin->roperand, emitter);
      return;
    }
    case KLAST_EXPR_PRE: {
      kl_assert(klgen_stacktop(gen) > 0, "");
      ...
      emitter->generic(gen, 1, obj, method, obj, 0, klgen_astposition(pattern));
      KlAstPre* pre = klcast(KlAstPre*, pattern);
      gen_pattern_fast(gen, pre->operand, emitter);
      return;
    }
    case KLAST_EXPR_CALL: {
      kl_assert(klgen_stacktop(gen) > 0, "");
      ...
      klgen_stackfree(gen, obj + nelem);
      if (nelem == 0) return;
      gen_pattern_fast(gen, args->exprs[nelem - 1], emitter);
      return;
    }
    case KLAST_EXPR_INDEX:
    case KLAST_EXPR_DOT:
...
static bool gen_pattern_fast_check(KlGenUnit* gen, KlAstExpr* pattern) {
  switch (klast_kind(pattern)) {
    case KLAST_EXPR_TUPLE: {
      KlAstTuple* tuple = klcast(KlAstTuple*, pattern);
      KlAstExpr** exprs = tuple->vals;
      ...
      return allislval(exprs, nexpr - 1) &&
             gen_pattern_fast_check(gen, exprs[nexpr - 1]);
    }
    case KLAST_EXPR_ARRAY: {
      KlAstExprList* exprlist = klcast(KlAstArray*, pattern)->exprlist;
      size_t nelem = exprlist->nexpr;
      ...
      return gen_pattern_fast_check(gen, elems[i]);
    }
    case KLAST_EXPR_CLASS: {
      KlAstClass* klclass = klcast(KlAstClass*, pattern);
      size_t nval = klclass->nfield;
      ...
      return gen_pattern_fast_check(gen, vals[nval - 1]);
    }
    case KLAST_EXPR_MAP: {
      KlAstMap* map = klcast(KlAstMap*, pattern);
    ...
    case KLAST_EXPR_WALRUS: {
      KlAstWalrus* walrus = klcast(KlAstWalrus*, pattern);
      if (!islval(walrus->pattern))
        return false;
      return gen_pattern_fast_check(gen, walrus->rval);
    }
    case KLAST_EXPR_BIN: {
      KlAstBin* bin = klcast(KlAstBin*, pattern);
      if (!islval(bin->loperand))
        return false;
      return gen_pattern_fast_check(gen, bin->roperand);
    }
    case KLAST_EXPR_PRE: {
      KlAstPre* pre = klcast(KlAstPre*, pattern);
      return gen_pattern_fast_check(gen, pre->operand);
    }
    case KLAST_EXPR_CALL: {
      ...
      return gen_pattern_fast_check(gen, elems[nelem - 1]);
    }
    case KLAST_EXPR_INDEX:
    case KLAST_EXPR_DOT:
    case KLAST_EXPR_WILDCARD:
...
void klgen_pattern_tostktop(KlGenUnit* gen, KlAstExpr* pattern, KlCStkId val, KlPatternEmitter* emitter) {
  ...
  if (gen_pattern_fast_check(gen, pattern)) {
    ...
    gen_pattern(gen, pattern, stktop + nres, emitter);
  }
}
...
bool klgen_pattern_fastmatching(KlGenUnit* gen, KlAstExpr* pattern) {
  if (!gen_pattern_fast_check(gen, pattern))
    return false;
  gen_pattern_fast(gen, pattern, &matchers);
  return true;
}

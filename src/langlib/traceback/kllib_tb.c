...
static unsigned locateline(FILE* input, long offset);
static bool showline_withcurl(KlFmtConfig* fmt, FILE* err, FILE* input, long begin, long end);

static void printCframe(KlFmtConfig* fmt, FILE* err);
static void printKframe_noany(KlFmtConfig* fmt, FILE* err);
static void printKframe_noposinfo(KlFmtConfig* fmt, FILE* err, const char* inputname);
static void printKframe_nosrcfile(KlFmtConfig* fmt, FILE* err, long begin, long end);
static void printKframe_openfilefailure(KlFmtConfig* fmt, FILE* err, const char* inputname, long begin, long end);
static void printKframe(KlFmtConfig* fmt, FILE* err, FILE* input, const char* inputname, long begin, long end);

static void printframe(KlFmtConfig* fmt, FILE* err, const KlCallInfo* callinfo);
static void dotraceback(KlFmtConfig* fmt, FILE* err, const KlCallInfo* from, const KlCallInfo* end);
...
static KlException tb_main(KlState* state) {
   ...
   * instead of currci. */
  dotraceback(&fmtconfig, err, currci->prev, klstate_baseci(state));
...
static void printframe(KlFmtConfig* fmt, FILE* err, const KlCallInfo* callinfo) {
  ...
  if (!posinfo && !srcfile) {
    printKframe_noany(fmt, err);
    return;
  }
  if (!posinfo) {
    printKframe_noposinfo(fmt, err, klstring_content(srcfile));
    return;
  }
  if (!srcfile) {
    KlKFuncFilePosition pos = posinfo[(callinfo->savedpc - klkfunc_entrypoint(kfunc)) - 1];
    printKframe_nosrcfile(fmt, err, pos.begin, pos.end);
    return;
  }
  FILE* input = fopen(klstring_content(srcfile), "rb");
  if (kl_unlikely(!input)) {
    KlKFuncFilePosition pos = posinfo[(callinfo->savedpc - klkfunc_entrypoint(kfunc)) - 1];
    printKframe_openfilefailure(fmt, err, klstring_content(srcfile), pos.begin, pos.end);
    return;
  }
  KlKFuncFilePosition pos = posinfo[(callinfo->savedpc - klkfunc_entrypoint(kfunc)) - 1];
  printKframe(fmt, err, input, klstring_content(srcfile), pos.begin, pos.end);
  fclose(input);
  return;
}
...
static void printKframe(KlFmtConfig* fmt, FILE* err, FILE* input, const char* inputname, long begin, long end) {
  ...
  unsigned line = locateline(input, begin);
  ...
  while (nputline++ < fmt->maxtextline) {
    if (!showline_withcurl(fmt, err, input, begin, end)) {
      break;
    }
  }

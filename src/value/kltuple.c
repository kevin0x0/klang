...
static KlGCObject* propagate(KlTuple* tuple, KlMM* klmm, KlGCObject* gclist);
static void delete(KlTuple* tuple, KlMM* klmm);
...
KlTuple* kltuple_create(KlMM* klmm, const KlValue* vals, size_t nval) {
  ...
  tuple->nval = nval;
  klmm_gcobj_enable(klmm, klmm_to_gcobj(tuple), &gcvfunc);
  return tuple;
}

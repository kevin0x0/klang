#include "tokenizer_generator/include/finite_automaton/object_pool/graph_pool.h"
...
inline KevGraph* kev_graph_pool_allocate(void) {
  if (graph_pool) {
...
inline void kev_graph_pool_deallocate(KevGraph* graph) {
  if (graph) {

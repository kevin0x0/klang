...
KevFA* kev_nfa_to_dfa(KevFA** nfa_array, uint64_t** p_accept_state_mapping_array) {
  ...
  for (uint64_t i = 0; i < array_size; ++i)
    start_label = kev_fa_state_labeling(nfa_array[i], start_label);
...
static inline KevBitSet* kev_compute_start_state_set(KevFA** nfa_array, KevBitSet* closures, uint64_t state_number) {
  ...
  if (start_state_set) {
    KevFA** pnfa = nfa_array - 1;
    while (*++pnfa)
      kev_bitset_union(start_state_set, &closures[kev_fa_get_start_state(*pnfa)->attr]);
  }
...
static inline KevBitSet* kev_compute_accept_state_set(KevFA** nfa_array, uint64_t state_number) {
  ...
  if (accept_state_set) {
    KevFA** pnfa = nfa_array - 1;
    while (*++pnfa)
      kev_bitset_set(accept_state_set, kev_fa_get_accept_state(*pnfa)->attr);
  }
...
static uint64_t* kev_get_state_ownership_array(KevFA** nfa_array, uint64_t state_number) {
  ...
  if (ownership) {
    ...
    uint64_t count = 0;
    while (*pnfa) {
      KevGraphNode* node = kev_graph_get_nodes(&(*pnfa)->transition);
      while (node) {
        ownership[node->attr] = count;
        node = node->next;
      }
      count++;
      pnfa++;

...
bool kev_fa_init_copy(KevFA* fa, KevFA* src) {
  ...
  int64_t original_accept_symbol = src->accept_states->attr;
  src->start_state->attr = KEV_NFA_STATE_NAME_START_MARK;
  src->accept_states->attr = KEV_NFA_STATE_NAME_ACCEPT_MARK;
  if (!kev_graph_init_copy(&fa->transition, &src->transition)) {
    fa->start_state = NULL;
    fa->accept_states = NULL;
    src->start_state->attr = original_start_symbol;
    src->accept_states->attr = original_accept_symbol;
    return false;
  }
  src->start_state->attr = original_start_symbol;
  src->accept_states->attr = original_accept_symbol;
  ...
  while (count != 2) {
    if (node->attr == KEV_NFA_STATE_NAME_START_MARK) {
      ...
      src->accept_states = node;
      ++count;
    }
    node = node->next;
  }
  src->start_state->attr = KEV_NFA_STATE_NAME_PLACE_HOLDER;
  src->accept_states->attr = KEV_NFA_STATE_NAME_PLACE_HOLDER;
  return true;
}
...
uint64_t kev_fa_state_labeling(KevFA* fa, uint64_t start_number) {
